...
namespace DSIGE.Web.Controllers
{
    public class Ubicacion_LecturasController : Controller
    {
        ...
        [HttpPost]
        public string ListandoServicios_reporte()
        {
            object loDatos;
            try
            {
                NUbicacion_Lecturas obj_negocio = new NUbicacion_Lecturas();
                loDatos = obj_negocio.Capa_Negocio_Get_ListaServicios_reporte(((Sesion)Session["Session_Usuario_Acceso"]).usuario.usu_id);
                return _Serialize(loDatos, true);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public string ListandoOperariosServicio( int idServicio)
        {
            object loDatos;
            try
            {
                NUbicacion_Lecturas obj_negocio = new NUbicacion_Lecturas();
                loDatos = obj_negocio.Capa_Negocio_Get_ListaOperariosServicio(idServicio);
                return _Serialize(loDatos, true);
            }
            catch (Exception ex)
            {
