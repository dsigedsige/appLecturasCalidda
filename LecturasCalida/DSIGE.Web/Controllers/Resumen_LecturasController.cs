...
namespace DSIGE.Web.Controllers
{
    public class Resumen_LecturasController : Controller
    {
        ...
        [HttpPost]
        public string Reporte_DetalleLecturas_new(int id_local, int id_servicio, string fechaini, string fechaFin, int ipOperario, int categoriaLectura)
        {
            int _fila = 2;
            string FileRuta = "";
            string FileExcel = "";
            ...
            {
                NLectura obj_negocio = new NLectura();
                loDatos = obj_negocio.NListaLecturaDetalladoEstado_new(id_local, id_servicio, fechaini, fechaFin, ipOperario, categoriaLectura);
                ...
                idUsuario = ((Sesion)Session["Session_Usuario_Acceso"]).usuario.usu_id;

                FileRuta = System.Web.Hosting.HostingEnvironment.MapPath("~/Upload/" + idUsuario + "_Detalle_Resumen_Lecturas_new.xlsx");
                string rutaServer = ConfigurationManager.AppSettings["servidor-archivos"];


                FileExcel = rutaServer + idUsuario + "_Detalle_Resumen_Lecturas_new.xlsx";
                FileInfo _file = new FileInfo(FileRuta);
                if (_file.Exists)
                {
                    _file.Delete();
                ...
                using (Excel.ExcelPackage oEx = new Excel.ExcelPackage(_file))
                {
                    ...
                    foreach (DataRow oBj in loDatos.Rows)
                    {
                        ac += 1;
                        ...
                        oWs.Cells[_fila, 7].Value = oBj["Lectura"].ToString();
                        oWs.Cells[_fila, 8].Value = oBj["Cod_Observacion"].ToString();
                        oWs.Cells[_fila, 9].Value = oBj["Comentario"].ToString();
                        _fila++;
                    }
