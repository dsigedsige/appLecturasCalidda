...
namespace DSIGE.Dato
{
    public class DLectura
    {
        ...
        public List<LecturaHistorico> DListaLecturaResumenObs(Request_Lectura_Historico oRq, string lista)
        {
            try
            {
                ...
                else if (total == 1)
                {
                    ...
                    {
                        cn.Open();
                        using (SqlCommand cmd = new SqlCommand("DSIGE_Reporte_Resumen_Oper_Obs", cn))
                        {
                            cmd.CommandTimeout = 0;
                            cmd.CommandType = CommandType.StoredProcedure;
                            cmd.Parameters.Add("@Local", SqlDbType.VarChar).Value = oRq.local;
                            ...
                            using (SqlDataReader dr = cmd.ExecuteReader())
                            {
                                while (dr.Read())
                                {
                                    LecturaHistorico obj_entidad = new LecturaHistorico();

                                    obj_entidad.des_ope = dr["Operario"].ToString();
                                    obj_entidad.obs = dr["OBS"].ToString();
                                    obj_entidad.total = Convert.ToDouble(dr["Total"].ToString());
                                    obj_entidad.sinFoto = Convert.ToInt32(dr["SinFoto"].ToString());
                                    obj_entidad.conFoto = Convert.ToDouble(dr["ConFoto"].ToString());
        ...
        public DataTable DListaLecturaDetalladaEstado_New(int id_local, int id_servicio, string fechaini, string fechaFin, int id_operario, int categoriaLectura)
        {
            DataTable dt_detalle = new DataTable();
            try
            {
                ...
                using (SqlConnection cn = new SqlConnection(cadenaCnx))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_S_RESUMEN_LECTURA_DET_ESTADO_NEW", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@Local", SqlDbType.Int).Value = id_local;
        ...
        public List<LecturaTomadas> DListaLecturaTomadaFoto(Request_Lectura_Tomadas oRq, string lista, int id_operario)
        {
            try
            {
                ...
                else if (total == 1)
                {
                    ...
                    {
                        cn.Open();
                        using (SqlCommand cmd = new SqlCommand("DSIGE_Lista_Lecturas_tomadas_Foto", cn))
                        {
                            cmd.CommandTimeout = 0;
                            cmd.CommandType = CommandType.StoredProcedure;
                            ...
                            using (SqlDataReader iDr = cmd.ExecuteReader())
                            {
                                while (iDr.Read())
                                {
                                    LecturaTomadas obj_entidad = new LecturaTomadas();

                                   obj_entidad.id_lectura = Convert.ToInt32(iDr["id_Lectura"]);
                                   obj_entidad.orden_lectura = Convert.ToInt32(iDr["orden_lectura"]);
                                   obj_entidad.suministro_lectura = Convert.ToString(iDr["suministro_lectura"]);
                                   obj_entidad.medidor_lectura = Convert.ToString(iDr["medidor_lectura"]);
                                   obj_entidad.marcaMedidor_lectura = Convert.ToString(iDr["marcaMedidor_lectura"]);
                                   obj_entidad.direccion_lectura = Convert.ToString(iDr["direccion_lectura"]);
                                   obj_entidad.lectura_movil = Convert.ToString(iDr["LecturaMovil_Lectura"]);
                                   obj_entidad.tieneFoto_lectura = Convert.ToString(iDr["tieneFoto_lectura"]);
                                   obj_entidad.operario = Convert.ToString(iDr["Operario"]);
                                   obj_entidad.fechaLectura = Convert.ToString(iDr["fechaLecturaMovil_lectura"]);
                                   obj_entidad.obs = Convert.ToString(iDr["OBS"]);
                                   obj_entidad.ubicacion = Convert.ToString(iDr["Ubicacion"]);
                                   obj_entidad.notas = Convert.ToString(iDr["Observacion_lectura"]);
                                   obj_entidad.latitud_lectura = Convert.ToString(iDr["latitud_lectura"]);
                                   obj_entidad.longitud_lectura = Convert.ToString(iDr["longitud_lectura"]);
        ...
        /// <param name="oRq"></param>
        /// <returns></returns>
        public List<LecturaTomadas> DListaLecturaHistoricoXSuministro_ant(Request_Lectura_Tomadas oRq)
        {
            try
            {
                List<LecturaTomadas> oLs = new List<LecturaTomadas>();
            ...
            try
            {
               string cadenaCnx = System.Configuration.ConfigurationManager.ConnectionStrings["dataSige"].ConnectionString;
                ...
                using (SqlConnection cn = new SqlConnection(cadenaCnx))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_S_LECTURAS_CORTES_HISTORICO_II", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@Local", SqlDbType.Int).Value = oRq.local;
                        ...
                        cmd.Parameters.Add("@Suministro", SqlDbType.VarChar).Value = oRq.suministro;
                        cmd.Parameters.Add("@Medidor", SqlDbType.VarChar).Value = oRq.medidor;
                        cmd.Parameters.Add("@tipoServico", SqlDbType.Int).Value = Servicios[0];
                        ...
                        using (SqlDataReader iDr = cmd.ExecuteReader())
                        {
                            while (iDr.Read())
                            {
                                ...
                                obj_entidad.tieneFoto_lectura = Convert.ToString(iDr["tieneFoto_lectura"]);
                                obj_entidad.operario = Convert.ToString(iDr["Operario"]);
                                obj_entidad.fechaAsignacion_Lectura = Convert.ToString(iDr["fechaAsignacion_Lectura"]);
                                obj_entidad.fechaLectura = Convert.ToString(iDr["fechaLecturaMovil_lectura"]);
                                obj_entidad.obs = Convert.ToString(iDr["OBS"]);
                                obj_entidad.notas = Convert.ToString(iDr["Observacion_lectura"]);
                                obj_entidad.id_TipoServicio = Convert.ToInt32(iDr["id_TipoServicio"]);
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_S_LISTA_LECTURAS_TOMADAS_NEW", cn))
                    {
                        ...
                        using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                        {
                            ...
                            foreach (DataRow Fila in dt_detalle.Rows)
                            {
                                ...
                                obj_entidad.horaLecturaMovil_Lectura = Fila["horaLecturaMovil_Lectura"].ToString();
